...
local result = engine.load_library("pr_nsight_aftermath")
if(result ~= true) then
    -- Module failed to load
    console.print_warning("Failed to load \"Nsight Aftermath\" module: " .. r)
    return
end

-- The default module defines several demo Lua bindings in "src/pr_module.cpp", we can call them like so:
pr_nsight_aftermath.print()

local d = pr_nsight_aftermath.DemoClass()
